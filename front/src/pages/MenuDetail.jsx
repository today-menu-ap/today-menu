...
export default function MenuDetail() {
  ...
  if (!rest) return <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-muted)' }}>로딩 중...</div>

  const stars = '★'.repeat(Math.min(Math.round(rest.avg_rating ?? 0), 5)).padEnd(5, '☆')

  return (
    <>
      <Link to="/menu" className="btn btn-sm btn-secondary" style={{ marginBottom: 16 }}>← 목록으로</Link>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--bg-white)', border: '1px solid var(--border-color)', borderRadius: 'var(--border-radius-xl)', overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
                ...
                <span style={{ color: 'var(--text-muted)', fontSize: '.88rem' }}>리뷰 {Math.floor((rest.avg_rating ?? 0) * 20)}개</span>
              </div>
              <p style={{ color: 'var(--text-muted)', fontSize: '.9rem', marginBottom: 4 }}>📍 {rest.address}</p>
              {rest.description && (
                <p style={{ marginTop: 12, fontSize: '.9rem', lineHeight: 1.7, color: 'var(--text-secondary)' }}>{rest.description}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          <div style={{ background: 'var(--bg-white)', border: '1px solid var(--border-color)', borderRadius: 'var(--border-radius-xl)', padding: 24, marginBottom: 16 }}>
            ...
            {user ? (
              <>
                <Link to="/party/create" className="btn btn-primary btn-block">👥 밥친구 파티 만들기</Link>
                <hr className="divider" />
                <Link to="/party" className="btn btn-secondary btn-block">모집 중 파티 보기</Link>
              </>
            ) : (
              <Link to="/login" className="btn btn-primary btn-block">로그인 후 파티 참여</Link>
            )}
          </div>
          ...
          <div style={{ background: 'var(--bg-white)', border: '1px solid var(--border-color)', borderRadius: 'var(--border-radius-xl)', padding: 24 }}>
            ...
            <p style={{ fontSize: '.88rem', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 12 }}>{rest.address}</p>
